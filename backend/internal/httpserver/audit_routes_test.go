...
func TestRegisterRoutesAuditListReturnsUserEvents(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/audit?page=1&page_size=10", nil)
	...
	req = httptest.NewRequest(http.MethodGet, "/api/audit?page=0", nil)
	req.AddCookie(sessionCookie)
	rec = httptest.NewRecorder()
	router.ServeHTTP(rec, req)
...
func TestRegisterRoutesAuditListIncludesRecoveryRequests(t *testing.T) {
	t.Run("email verification request", func(t *testing.T) {
		...
		registerReq := httptest.NewRequest(http.MethodPost, "/api/auth/register", bytes.NewBufferString(`{"email":"verify@example.test","password":"correct horse battery staple"}`))
		...
		verifyReq := httptest.NewRequest(http.MethodGet, "/api/auth/email/verification?email=verify@example.test", nil)
	...
	t.Run("password reset request", func(t *testing.T) {
		...
		registerReq := httptest.NewRequest(http.MethodPost, "/api/auth/register", bytes.NewBufferString(`{"email":"reset@example.test","password":"correct horse battery staple"}`))
		registerReq.Header.Set("Content-Type", "application/json")
		registerRec := httptest.NewRecorder()
		router.ServeHTTP(registerRec, registerReq)
		require.Equal(t, http.StatusCreated, registerRec.Code)
		...
		resetReq := httptest.NewRequest(http.MethodPost, "/api/auth/password-reset/request", bytes.NewBufferString(`{"email":"reset@example.test"}`))
...
func TestRegisterRoutesAuditListIncludesLedgerAndImportMutations(t *testing.T) {
	...
	bookReq := httptest.NewRequest(http.MethodPost, "/api/books", bytes.NewBufferString(`{"name":"Household","reportingCurrency":"usd"}`))
	...
	updateBookReq := httptest.NewRequest(http.MethodPatch, "/api/books/"+book.ID, bytes.NewBufferString(`{"name":"Household 2026"}`))
	...
	updateGroupReq := httptest.NewRequest(http.MethodPatch, "/api/accounts/groups/"+group.ID, bytes.NewBufferString(`{"name":"Daily Wallets"}`))
	...
	accountReq := httptest.NewRequest(http.MethodPost, "/api/accounts", bytes.NewBufferString(`{"groupId":"`+group.ID+`","name":"Cash","type":"cash","currency":"usd","sharedBookIds":["`+book.ID+`"],"openingBalanceCents":0}`))
	...
	categoryReq := httptest.NewRequest(http.MethodPost, "/api/books/"+book.ID+"/categories", bytes.NewBufferString(`{"name":"Dining","direction":"expense","sortOrder":1,"rawSourceName":"Food"}`))
	...
	updateCategoryReq := httptest.NewRequest(http.MethodPatch, "/api/books/"+book.ID+"/categories/"+category.ID, bytes.NewBufferString(`{"name":"Dining Out"}`))
	...
	entryReq := httptest.NewRequest(http.MethodPost, "/api/books/"+book.ID+"/entries", bytes.NewBufferString(`{"type":"expense","accountId":"`+account.ID+`","categoryId":"`+category.ID+`","amountCents":1230,"transactionCurrency":"usd","occurredAt":"2026-07-01T12:00:00Z","note":"Lunch","tags":["food"]}`))
	...
	updateEntryReq := httptest.NewRequest(http.MethodPatch, "/api/books/"+book.ID+"/entries/"+entry.ID, bytes.NewBufferString(`{"note":"Updated lunch"}`))
	...
	deleteEntryReq := httptest.NewRequest(http.MethodDelete, "/api/books/"+book.ID+"/entries/"+entry.ID, nil)
	...
	auditReq := httptest.NewRequest(http.MethodGet, "/api/audit?page=1&page_size=50", nil)
