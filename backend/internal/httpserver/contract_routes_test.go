...
func TestRegisterRoutesOpenAPIContract(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "health", method: http.MethodGet, path: "/api/v1/health", status: http.StatusOK},
		{name: "runtime config", method: http.MethodGet, path: "/api/v1/runtime-config", status: http.StatusOK},
		{
			name:   "register",
		...
		},
		{
			name:   "books",
			method: http.MethodGet,
			path:   "/api/v1/books?page=1&page_size=20",
			cookie: sessionCookie,
			status: http.StatusOK,
		},
		{
			name:   "book",
			method: http.MethodGet,
			path:   "/api/v1/books/book-household",
			cookie: sessionCookie,
			status: http.StatusOK,
		},
		{
		...
		},
		{
			name:   "categories",
			method: http.MethodGet,
			path:   "/api/v1/books/book-household/categories?page=1&page_size=50",
			cookie: sessionCookie,
			status: http.StatusOK,
		},
		{
			...
			method: http.MethodPost,
			path:   "/api/v1/books/book-household/entries",
