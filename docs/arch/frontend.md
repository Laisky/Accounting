...
- Reports load server-owned inputs through `useReportFoundationQuery` and `useReportBookDataQuery`,
  keyed by `queryKeys.reports.foundation(revision)` and
  `queryKeys.reports.bookData(bookId, revision)`. `ReportWorkspace` owns only report UI state such
  as selected book, filters, period, expanded panels, and filter-panel disclosure.
...
## Architecture overhaul (2026-07, implemented — see docs/proposals/archives/2026-07-06-architecture-overhaul.md)

- State ownership: server state lives in per-domain TanStack Query hooks (`hooks/useBooks`,
  `useAccounts`, `useEntries`, `useCategories`, `useBookMembers`, `useUserProfile`, `useReports`,
  `useAudit`, `usePasskeys`); mutations `invalidateQueries` by the keys in `hooks/queryKeys.ts`.
  Client state lives in narrow contexts: `SessionContext` (actor/runtimeConfig/logout),
  `BookContext` (selected book + derived reporting/display currency + rate index),
  `NoticeContext` (status/error banners + 10s undo), `ThemeContext` (writes `data-theme`).
- Shell/routing: `features/shell/MobileShell.tsx` is a layout route composing the providers and
  rendering header + notices + `<Outlet/>` + bottom nav. `useShellChrome` derives the active
  tab / detail ids from React Router `useMatch` (no manual path parsing). Route bodies are
  lazy-loaded from `features/shell/routes/`. Two `AppErrorBoundary` layers (app root + route).
- Entries: one canonical full-ledger query `entries.all(bookId)` backs search / account detail /
  entry detail / reports; `entries.recent(bookId)` backs home/record. No duplicate full loads.
- Tokens/CSS: `styles/palette.css` (generated by `scripts/tokenize-colors.mjs`) holds every
  primitive `--tone-*` color; `styles/tokens.css` maps them to semantic roles + light/dark under
  `:root[data-theme="dark"]` + spacing/radius/shadow/type scales; `styles/layers.css` declares
  `@layer tokens, base, components, features`. No raw color literals outside the token files
  (`pnpm run check:tokens` + a stylelint `function-disallowed-list` gate enforce it).
- UI primitives: `components/ui/{Button,Card,ConfirmDialog,EmptyState,FormField,Notice,Sheet}`
  (tested). `Sheet` owns focus trap / Escape / return-focus. Destructive actions use
  `ConfirmDialog` + the `NoticeContext` undo window.
- Keyboard: `hooks/useGlobalShortcuts` (`n` new entry, `/` search, `?` help, Esc closes sheets).
- Telemetry: `lib/telemetry.ts` is the only module besides `apiClient` allowed to touch the
  network; it sends a sanitized allowlist payload (hashed messages, no amounts/notes/emails) via
