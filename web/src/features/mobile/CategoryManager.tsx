...
export function CategoryManager({
  ...
  const sortedCategories = [...categories].sort(
    (left, right) => Number(left.archived) - Number(right.archived) || left.name.localeCompare(right.name),
  );
    ...
    <section className="categoryManager" aria-label={t('mobile.categories.title')}>
      ...
      <form className="categoryCreateForm" aria-label={t('mobile.categories.create')} onSubmit={handleCreate}>
        ...
        <label>
          ...
          <select
            aria-label={t('mobile.categories.direction')}
            value={direction}
            disabled={!canManageCategories}
        ...
        <button type="submit" disabled={!canManageCategories || isBusy || !name.trim()}>
          {t('mobile.categories.create')}
        </button>
      </form>
...
function CategoryEditor({
  ...
  const hasChanges =
    ...
    draft.direction !== category.direction ||
    draft.archived !== category.archived;
    ...
    <li className={draft.archived ? 'categoryArchived' : ''}>
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) =>
            setDraft((current) => ({ ...current, direction: event.target.value as CategoryDirection }))
          }
        >
          <option value="expense">{t('mobile.record.types.expense')}</option>
          <option value="income">{t('mobile.record.types.income')}</option>
      ...
      <div className="categoryEditorActions">
        <button
          ...
          disabled={!canManageCategories || isBusy}
