...
export async function fetchUserProfile(signal?: AbortSignal): Promise<AuthUser> {
  const payload = await apiRequest<Schemas['UserEnvelope']>('/api/users/me', { signal });
  return payload.user;
}
...
export async function updateUserProfile(input: { baseCurrency?: string }): Promise<AuthUser> {
  const payload = await apiRequest<Schemas['UserEnvelope']>('/api/users/me', { method: 'PATCH', body: input });
  return payload.user;
}
...
export async function fetchSession(signal?: AbortSignal): Promise<SessionResult> {
  return apiRequest<SessionResult>('/api/auth/session', { signal });
}
...
export async function loginWithPassword(
  ...
  const payload = await apiRequest<Schemas['AuthResult']>('/api/auth/login', {
    method: 'POST',
    body: {
      email,
      password,
...
export async function registerWithPassword(
  ...
  return apiRequest<Schemas['UserEnvelope']>('/api/auth/register', {
    method: 'POST',
...
export async function requestEmailVerification(email: string): Promise<void> {
  await apiRequest<Schemas['AcceptedResponse']>(`/api/auth/email/verification?email=${encodeURIComponent(email)}`);
}
...
export async function confirmEmailVerification(email: string, code: string): Promise<Schemas['UserEnvelope']> {
  return apiRequest<Schemas['UserEnvelope']>('/api/auth/email/verification', {
    method: 'POST',
    body: { email, code },
  });
}
...
export async function requestPasswordReset(email: string): Promise<void> {
  await apiRequest<Schemas['AcceptedResponse']>('/api/auth/password-reset/request', {
    method: 'POST',
    body: { email },
  });
}
...
export async function confirmPasswordReset(
  ...
  return apiRequest<Schemas['UserEnvelope']>('/api/auth/password-reset/confirm', {
    method: 'POST',
...
export async function beginPasskeyLogin(): Promise<PasskeyStart> {
  return apiRequest<PasskeyStart>('/api/auth/passkeys/login/begin', { method: 'POST' });
}
...
export async function finishPasskeyLogin(flowId: string, credential: unknown): Promise<AuthResult> {
  return apiRequest<AuthResult>('/api/auth/passkeys/login/finish', { method: 'POST', body: { flowId, credential } });
}
...
export async function fetchPasskeys(signal?: AbortSignal): Promise<PasskeyList> {
  return apiRequest<PasskeyList>('/api/auth/passkeys?page=1&page_size=20', { signal });
}
...
export async function beginPasskeyRegistration(): Promise<PasskeyStart> {
  return apiRequest<PasskeyStart>('/api/auth/passkeys/register/begin', { method: 'POST' });
}
...
export async function finishPasskeyRegistration(
  ...
  return apiRequest<PasskeyListItem>('/api/auth/passkeys/register/finish', {
    method: 'POST',
...
export async function updatePasskey(passkeyId: string, label: string): Promise<PasskeyListItem> {
  return apiRequest<PasskeyListItem>(`/api/auth/passkeys/${encodeURIComponent(passkeyId)}`, {
    method: 'PUT',
    body: { label },
  });
}
...
export async function deletePasskey(passkeyId: string): Promise<void> {
  await apiRequest<void>(`/api/auth/passkeys/${encodeURIComponent(passkeyId)}`, { method: 'DELETE' });
}
...
export async function fetchTotpStatus(signal?: AbortSignal): Promise<TotpStatus> {
  return apiRequest<TotpStatus>('/api/auth/totp/status', { signal });
}
...
export async function setupTotp(): Promise<TotpSetup> {
  return apiRequest<TotpSetup>('/api/auth/totp/setup', { method: 'POST' });
}
...
export async function confirmTotp(code: string): Promise<TotpStatus> {
  return apiRequest<TotpStatus>('/api/auth/totp/confirm', { method: 'POST', body: { code } });
}
...
export async function disableTotp(code: string): Promise<TotpStatus> {
  return apiRequest<TotpStatus>('/api/auth/totp/disable', { method: 'POST', body: { code } });
}
