...
export async function fetchLedgerSummary(signal: AbortSignal): Promise<LedgerSummary> {
  return apiRequest<LedgerSummary>('/api/ledger/summary', { signal });
}
...
export async function fetchBooks(): Promise<BookListItem[]> {
  const list = await apiRequest<Schemas['BookList']>('/api/books?page=1&page_size=50');
  return list.items;
}
...
export async function createBook(name: string, reportingCurrency: string): Promise<BookListItem> {
  return apiRequest<BookListItem>('/api/books', { method: 'POST', body: { name, reportingCurrency } });
}
...
export async function updateBook(
  ...
  return apiRequest<BookListItem>(`/api/books/${encodeURIComponent(bookId)}`, { method: 'PATCH', body: input });
}
...
export async function fetchBookMembers(bookId: string): Promise<BookMember[]> {
  const list = await apiRequest<Schemas['BookMemberList']>(
    `/api/books/${encodeURIComponent(bookId)}/members?page=1&page_size=50`,
  );
  return list.items;
}
...
export async function fetchAccountGroups(): Promise<AccountGroup[]> {
  const list = await apiRequest<Schemas['AccountGroupList']>('/api/accounts/groups?page=1&page_size=50');
  return list.items;
}
...
export async function createAccountGroup(name: string): Promise<AccountGroup> {
  return apiRequest<AccountGroup>('/api/accounts/groups', { method: 'POST', body: { name, sortOrder: 0 } });
}
...
export async function updateAccountGroup(groupId: string, input: AccountGroupUpdateInput): Promise<AccountGroup> {
  return apiRequest<AccountGroup>(`/api/accounts/groups/${encodeURIComponent(groupId)}`, {
    method: 'PATCH',
    body: input,
  });
}
...
export async function fetchAccounts(): Promise<Account[]> {
  const list = await apiRequest<Schemas['AccountList']>('/api/accounts?page=1&page_size=50');
  return list.items;
}
...
export async function fetchCategories(bookId: string): Promise<Category[]> {
  const list = await apiRequest<Schemas['CategoryList']>(
    `/api/books/${encodeURIComponent(bookId)}/categories?page=1&page_size=50`,
  );
  return list.items;
}
...
export async function createCategory(bookId: string, input: CategoryCreateInput): Promise<Category> {
  return apiRequest<Category>(`/api/books/${encodeURIComponent(bookId)}/categories`, {
    method: 'POST',
...
export async function updateCategory(
  ...
  return apiRequest<Category>(`/api/books/${encodeURIComponent(bookId)}/categories/${encodeURIComponent(categoryId)}`, {
    method: 'PATCH',
    body: input,
  });
}
...
export async function fetchEntries(bookId: string): Promise<EntryList> {
  return apiRequest<EntryList>(`/api/books/${encodeURIComponent(bookId)}/entries?page=1&page_size=20`);
}
...
export async function fetchAllEntries(bookId: string): Promise<Entry[]> {
  ...
  do {
    const list = await apiRequest<EntryList>(
      `/api/books/${encodeURIComponent(bookId)}/entries?page=${page}&page_size=${pageSize}`,
    );
...
export async function createEntry(bookId: string, input: Schemas['EntryCreateRequest']): Promise<Entry> {
  return apiRequest<Entry>(`/api/books/${encodeURIComponent(bookId)}/entries`, { method: 'POST', body: input });
}
...
export async function updateEntry(bookId: string, entryId: string, input: EntryUpdateInput): Promise<Entry> {
  return apiRequest<Entry>(`/api/books/${encodeURIComponent(bookId)}/entries/${encodeURIComponent(entryId)}`, {
    method: 'PATCH',
    body: input,
  });
}
...
export async function deleteEntry(bookId: string, entryId: string): Promise<void> {
  await apiRequest<void>(`/api/books/${encodeURIComponent(bookId)}/entries/${encodeURIComponent(entryId)}`, {
    method: 'DELETE',
  });
}
