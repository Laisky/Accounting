...
export async function registerAndSignIn(page: Page, prefix = 'e2e'): Promise<string> {
  ...
  const register = await page.request.post('/api/v1/auth/register', { data: { email, password: PASSWORD } });
  expect(register.status()).toBe(201);
  const login = await page.request.post('/api/v1/auth/login', { data: { email, password: PASSWORD } });
